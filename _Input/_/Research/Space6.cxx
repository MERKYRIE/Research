...
namespace NResearch::NResearch
{
    SSpace6::SSpace6()
    {
        ...
        for(FIterationX = 0 ; FIterationX < FWidth ; FIterationX++)
        {
            ...
            {
                FGrounds[FIterationX][FIterationY] = std::make_unique<NSpace6::SGround>();
            }
            FGrounds[FIterationX].shrink_to_fit();
        }
        FGrounds.shrink_to_fit();
        FEntities[0][0] = std::make_unique<NSpace6::SEntity>();
        FX = 0.0;
        FY = 0.0;
        FScaleX = 0.05;
        FScaleY = 0.05;
        FScaleLastX = FScaleX;
        FScaleLastY = FScaleY;
        FImage = std::make_unique<NSpace6::SImage>();
    }

    void SSpace6::IUpdate()
    {
        FScaleLastX = FScaleX;
        FScaleLastY = FScaleY;
