...
namespace NResearch::NResearch
{
    SSpace2::SSpace2()
    {
        ...
        for(FIterationX = 0 ; FIterationX < FWidth ; FIterationX++)
        {
            ...
            {
                FMatrix[FIterationX][FIterationY] = std::make_shared<NSpace2::SPartition>();
                FMatrix[FIterationX][FIterationY]->FGround = std::make_shared<NSpace2::SGround>();
            }
            FMatrix[FIterationX].shrink_to_fit();
        }
        FMatrix.shrink_to_fit();
        FMatrix[0][0]->FEntity = std::make_shared<NSpace2::SEntity>();
        FX = 0.0;
        FY = 0.0;
        FScaleX = 0.05;
        FScaleY = 0.05;
