...
namespace NResearch::NResearch
{
    SSpace7::SSpace7()
    {
        ...
        for(FIterationX = 0 ; FIterationX < FWidth ; FIterationX++)
        {
            ...
            {
                FGrounds[FIterationX][FIterationY] = std::make_unique<NSpace7::SGround>();
            }
            FGrounds[FIterationX].shrink_to_fit();
        }
        FGrounds.shrink_to_fit();
        FEntities[0][0] = std::make_unique<NSpace7::SEntity>();
        FX = 0.0;
        FY = 0.0;
        FScaleX = 0.05;
        FScaleY = 0.05;
        FScaleLastX = FScaleX;
        FScaleLastY = FScaleY;
        FImage = std::make_unique<NSpace7::SImage>();
    }

    void SSpace7::IUpdate()
    {
        FScaleLastX = FScaleX;
        FScaleLastY = FScaleY;
