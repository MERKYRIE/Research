...
namespace NResearch::NResearch
{
    SSpace4::SSpace4()
    {
        ...
        for(FIterationX = 0 ; FIterationX < FWidth ; FIterationX++)
        {
            ...
            {
                FMatrix[FIterationX][FIterationY] = std::make_unique<NSpace4::SPartition>();
                FMatrix[FIterationX][FIterationY]->FGround = std::make_unique<NSpace4::SGround>();
            }
            FMatrix[FIterationX].shrink_to_fit();
        }
        FMatrix.shrink_to_fit();
        FMatrix[0][0]->FEntity = std::make_unique<NSpace4::SEntity>();
        FX = 0.0;
        FY = 0.0;
        FScaleX = 0.05;
        FScaleY = 0.05;
